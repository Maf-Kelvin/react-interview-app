...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mockAuthenticate(phone.trim())) {
      ...
      setServerError("This number is not registered. Use the demo credentials below.");
    }
    setLoading(false);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-slate-100 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-md p-10">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800 leading-none">UserHub</h1>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="mb-5">
            ...
            />
            {validationError && (
              <p id="phone-error" role="alert" className="text-red-500 text-xs mt-1.5 flex items-center gap-1">
        ...
        <div className="mt-6 bg-slate-50 rounded-xl p-4 text-sm text-slate-500">
          ...
          <p>Phone: <code className="bg-slate-200 px-1.5 py-0.5 rounded text-slate-700 text-xs">+254712345678</code></p>
        </div>
      </div>
    </div>
  );
